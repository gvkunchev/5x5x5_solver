...
#    v4: Writes all solutions to a file                               #
#        Some of the prints are removed                               #
#                                                                     #
...
# Will hold a file object for the log and all solutions
logFile = None
solutionFile = None
...
# Also handles the file holding the solutions
try:
  logFile = open("cube_solver_log.dat","r+")
  solutionFile = open("cube_solver_solution.dat","a+")
  fileContent =  logFile.readlines()
  if fileContent:
    scriptLog = eval(fileContent[0])
  print "Log file loaded successfully. There are %s movements and %d solutions so far." %("{0:n}".format(scriptLog["Movements"]), scriptLog["Solutions"])
  #print "The log file consists of:\n"
  #print json.dumps(scriptLog, indent=4)
except Exception:
  # Create the file
  logFile = open("cube_solver_log.dat","w+")
  solutionFile = open("cube_solver_solution.dat","w+")
...
def solve():
  if findPath():
    print "Solved!"
    print getSolutionToString()
  else:
    print "Looks like all combinations have been checked."
    saveInformationToFile()

# Saves everything to files
# Used when paused or finished
def saveInformationToFile():
  logFile.seek(0)
  logFile.truncate(0)
...
# Saves the solution in the external file
def saveSolutionToFile():
  scriptLog["Solutions"] +=1
  print "New solution found (#%d):\n\n" %scriptLog["Solutions"]
  solutionFile.write("\n\n Solution #%d: \n\n" %scriptLog["Solutions"])
  solutionToString = getSolutionToString()
  solutionFile.write(solutionToString)
...
def findPath():
  ...
  if nextSpot:
    ...
    # Print current solution and continue working out the rest
    saveSolutionToFile()
    print getSolutionToString()
    playSound()
    x, y, z = [0, 0, 0]
...
# Alerts that a new solution was found
def playSound():
  musicParts = [
    {
      "sound":{
  ...
  for eachPart in musicParts:
    winsound.Beep(eachPart["sound"]["freq"],eachPart["sound"]["dur"])
    time.sleep(eachPart["sleep"])
